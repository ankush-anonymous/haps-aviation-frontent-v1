...
export default function Navigation() {
  ...
  if (pathname.startsWith("/admin") || pathname.startsWith("/mentee") || pathname.startsWith("/login")|| pathname.startsWith("/mentor")) {
    return null
  }
            ...
            {navigation.map((item) => (
              <Link
                ...
                className={`px-3 py-2 text-sm font-medium transition-colors ${
                  pathname === item.href ? "text-blue-300 border-b-2 border-blue-300" : "text-white hover:text-blue-300"
                }`}
              >
                {item.name}
              </Link>
  ...
  <div className="flex space-x-2">
    <Link href="/login">
      <Button variant="outline" className="border-blue-300 text-white hover:bg-blue-800 bg-transparent">
        Mentee Login
      </Button>
    </Link>
    ...
    <Link href="/register">
      <Button className="bg-blue-600 hover:bg-blue-700 text-white">Book Consultation</Button>
          ...
          <div className="md:hidden flex items-center">
            <button onClick={() => setIsOpen(!isOpen)} className="text-white hover:text-blue-300 focus:outline-none">
  ...
  {isLoggedIn ? (
    ...
    <div className="space-y-2">
      <Link href="/login" className="block">
        <Button variant="outline" className="w-full border-blue-300 text-white hover:bg-blue-800 bg-transparent">
          Mentee Login
        </Button>
      </Link>
      ...
      <Link href="/register" className="block">
        <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white">Book Consultation</Button>
              ...
              <div className="px-3 py-2 border-t border-blue-700">
                <Link href="/admin/login" className="block">
                  <Button variant="ghost" className="w-full text-left text-white hover:bg-blue-700 justify-start">
                    Admin Login
                  </Button>
                </Link>
              </div>
